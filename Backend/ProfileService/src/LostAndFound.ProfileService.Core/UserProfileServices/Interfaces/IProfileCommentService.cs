...
namespace LostAndFound.ProfileService.Core.UserProfileServices.Interfaces
{
    public interface IProfileCommentService
    {
        ...
        Task<CommentDataResponseDto> CreateProfileComment(string rawUserId, string username, CreateProfileCommentRequestDto commentRequestDto, Guid profileOwnerId);
        Task DeleteProfileComment(string rawUserId, Guid profileOwnerId);
        Task<(ProfileCommentsSectionResponseDto, PaginationMetadata)> GetProfileCommentsSection(string rawUserId, Guid profileOwnerId, int pageNumber, int pageSize);
    }
}
